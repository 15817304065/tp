...
class API
{
    ...
    //语音合成接口
    public static function tts($params)
    {
        $params['sign'] = Signature::getReqSign($params);
        $url            = Configer::API_URL_PATH . '/aai/aai_tta';
        $response       = HttpUtil::doHttpPost($url, $params);
        return $response;
    }
...
//身份证识别
    public static function Idcardocr($params)
    {
        if (!self::_is_base64($params['image'])) {
            $params['image'] = base64_encode($params['image']);
        }
        ...
        $params['sign'] = Signature::getReqSign($params);
        $url            = Configer::API_URL_PATH . '/ocr/ocr_idcardocr';
        $response       = HttpUtil::doHttpPost($url, $params);
        return $response;
    }

    //label_images 标签图片识别
    public static function labelimages($params)
    {
        if (!self::_is_base64($params['image'])) {
            $params['image'] = base64_encode($params['image']);
        }
        $params['sign'] = Signature::getReqSign($params);
        $url            = Configer::API_URL_PATH . '/image/image_tag';
        $response       = HttpUtil::doHttpPost($url, $params);
        return $response;
    }

    //label_images 暴恐图片识别
    public static function Horrorimages($params)
    {
        if (!self::_is_base64($params['image'])) {
            $params['image'] = base64_encode($params['image']);
        }
        $params['sign'] = Signature::getReqSign($params);
        $url            = Configer::API_URL_PATH . '/image/image_terrorism';
        $response       = HttpUtil::doHttpPost($url, $params);
        return $response;
    }
