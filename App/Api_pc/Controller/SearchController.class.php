...
class SearchController extends Controller
{
    ...
    public function add_api_num($name, $openid = "")
    {

        $api_arr = M('api_info_pc')->where(array('api_name' => $name))->find();

        if (empty($api_arr)) {
            return false;
        } else {
            M('api_info_pc')->where(array('api_name' => $name))->setInc('num');
        }
        if ($openid && $openid != "null") {
            $data['openid']    = $openid;
            $data['api_id']    = $api_arr['id'];
            $data['date_time'] = date('Ymd');
            $re                = M('interface_pc')->where($data)->find();

            if (empty($re)) {
                $data['num']       = 1;
                $data['date_time'] = date('Ymd');
                M('interface_pc')->add($data);
            } else {
                if ($re['num'] < 500) {
                    M('interface_pc')->where($data)->setInc('num');
                } else {
    ...
    //     }
    //     if ($openid && $openid != "null") {
    //         $re = M('interface_pc')->where(array('openid' => $openid))->find();
    //         if (empty($re)) {
    //             $arr = array(
    //                 array(
    //                     'api_id'    => $api_arr['id'],
    ...
    //                 );
    //             }
    //             $re = M('interface_pc')->where(array('openid' => $openid))->save(array('info' => json_encode($info)));
    //         }
    //     }
    // }
    public function add_errorcode($ismobile = false, $name, $error_msg, $error_code)
    {
        if (!$ismobile) {
            $date = date('Ymd');
            $data = array('api_name' => $name, 'error_code' => $error_code, 'error_msg' => $error_msg, 'date_time' => $date);
            $re   = M('errorcode_pc')->where($data)->find();
            // var_dump($data);die;
            if (!$re) {
                $data['num'] = 1;
                M('errorcode_pc')->add($data);
            } else {
                M('errorcode_pc')->where($data)->setInc('num');
            }
        }
        return true;
    }

    public function out_link()
    {
        $name = I('post.name');
        if (empty($name)) {
            apiResponse("0", "参数有误");
        ...
        if (empty($api_arr)) {
            ...
            M('api_info_pc')->where(array('api_name' => $name))->setInc('out_link');
            apiResponse("1", "success", '200', "");
        }
    }
    public function add_pv()
    {
        ...
        if (empty($api_arr)) {
            ...
            M('api_info_pc')->where(array('api_name' => $name))->setInc('pv');
            apiResponse("1", "success", '200', "");
        }
    }
