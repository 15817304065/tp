...
class SearchController extends Controller
{
...

    {
        ...
        if (!empty($_FILES['file'])) {
            ...
            if (!isset($result['result'])) {
                ...
                $re = M('api_info')->where('keywords like "%' . $search . '%" or title="' . $search . '"')->select();
    ...
    public function add_api_num2($name, $openid = "")
    {
        ...
        $api_arr = M('api_info')->where(array('api_name' => $name))->find();

        if (empty($api_arr)) {
            return false;
        } else {
            M('api_info')->where(array('api_name' => $name))->setInc('num');
        }
        if ($openid && $openid != "null") {
            $data['openid']    = $openid;
            $data['api_id']    = $api_arr['id'];
            $data['date_time'] = date('Ymd');
            $re                = M('interface')->where($data)->find();

            if (empty($re)) {
                $data['num']       = 1;
                $data['date_time'] = date('Ymd');
                M('interface')->add($data);
            } else {
                if ($re['num'] < 500) {
                    M('interface')->where($data)->setInc('num');
                } else {
    ...
    public function add_api_num($name, $openid = "", $max_num = 500)
    {
        ...
        if ($openid && $openid != "null") {
            $re = M('interface2')->where(array('openid' => $openid))->find();
            if (empty($re)) {
                $arr = array(
                    array(
                        'api_id'    => $api_arr['id'],
                ...
                $re = M('interface2')->where(array('openid' => $openid))->save(array('info' => json_encode($info)));
            }
        }
    }
