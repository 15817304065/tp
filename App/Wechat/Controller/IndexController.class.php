...
{
    // private $type = C('wechat_type');
    //旧版 2018.3.18后不再使用
    public function getwechat($type = TYPE)
    {

        if (!cookie('wechat_info')) {
            $wechat = new \Common\Util\Getwe($type);

            $wechat_info = $wechat->getwechat();

        } else {
            $wechat_info = unserialize(cookie('wechat_info'));
    ...
    {
        $wechat_info = $this->wechatoauth2(array("id" => 2, "cache" => "T", "scope" => "T"));
        var_dump($wechat_info);
    }
    // 新版授权 2018.3.18 启动
    public function wechatoauth2($config = array("id" => 1, "cache" => "T", "scope" => "T"))
    {
        if ($config['cache'] == T) {
        ...
        if (!$wechat_data) {
            ...
            if (!$code) {
                ...
                $url = "http://wx.issmart.com.cn/web/frame/Wechat/index.php?m=Inter&c=OAuth2&a=code&is_id=" . $config['id'] . "&is_rurl=" . urlencode($current_url) . $is_user;
                header("location:$url");
            } else {
                // 清除url上is_code参数
                ...
                $is_id  = I("get.is_id");
                $url    = "http://wx.issmart.com.cn/web/frame/Wechat/index.php?m=Inter&c=OAuth2&a=getuserinfo&is_id=" . $is_id . "&is_code=" . $code;
